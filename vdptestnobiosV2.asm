...
START:
	LD	SP, 0x0000	; Memory is from 0x8000 to 0xFFFF
	...
	LD	B, 7		
	LD	A, 0xF5		; White text on light blue
	CALL	VDP_WRITEREG
	...
	LD	HL, FONT_HEAD+1 
	; Copy the 128 * 8 -1 bytes from the fontto the pattern table
	LD	C, 4		; 8 * 128 bytes = 4 * 256 
...
FONTLOOP:
	...
	; Now fill the name table with the character set
	LD	HL, 0x0800	; Name table base
	LD	A, 0x00		; Start at 0
	CALL	VDP_WRITEADDR
	LD	B, 127
CHARLOOP:
	INC	A
	...
	; Fill the rest with blanks, 40x24 = 960 
	; 960 - 128 = 832. 832 = (3 * 256) + 64
	LD 	A, 0
	LD	C, 4
	LD	B, 64		; 3 * 256 + 64
BLANKLOOP:
